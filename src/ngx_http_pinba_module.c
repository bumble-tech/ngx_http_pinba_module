...
static ngx_command_t  ngx_http_pinba_commands[] = { /* {{{ */
    ...
    { ngx_string("pinba_enable"),
      ...
      offsetof(ngx_http_pinba_loc_conf_t, enable),
      NULL },
    ...
    { ngx_string("pinba_resolve_freq"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_pinba_loc_conf_t, resolve_freq),
      NULL },
    ...
    # endif
		...
		*sock = &element->sock;
		...
		if (cf) {
			...
			ngx_conf_log_error(
				NGX_LOG_DEBUG, cf, 0,
				"[pinba] server %s:%s re-resolve after %d sec",
				element->server_name, element->port, resolve_freq
			);
#endif
		} else {
			ngx_log_debug3(
...
static ngx_int_t ngx_http_pinba_handler(ngx_http_request_t *r) /* {{{ */
{
	...
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "[pinba] http handler");
